...
| Campo | Valor |
|---|---|
| Onda atual | 1 — Fundação **concluída**: VJT-001, VJT-002, VJT-003 e VJT-008 mergeados. Onda fechada. Próxima execução recomendada: **VJT-003b** (não VJT-004/VJT-007 — ambos dependem de catálogo de checklists completo, e VJT-007/VJT-009 estão bloqueados também por VJT-003b, ver Seção 7) |
| Último ticket concluído (mergeado) | VJT-003 (PR [#19](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/19), issue [#17](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/17), mergeado em 2026-07-24 16:51) — resolvendo a duplicidade encontrada na sessão anterior: issue [#16](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/16)/PR [#18](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/18) foram **fechados nesta sessão** (PR #18 `closed` sem merge, issue #16 `not_planned`), com comentário de justificativa em cada um. Comparação completa no Log (Seção 8) |
| Ticket em aberto aguardando review | — nenhum |
| Migration aplicada no Supabase | **Não verificável nesta sessão** (mesma limitação já registrada): o conector Supabase desta conta não acessa `urrlqljlibpzaqnemlwf` (Lovable Cloud). Agora há **duas migrations pendentes de aplicação manual** via SQL Editor: a do VJT-001 (contagens `paises_visto`=14/`checklist_templates`=15, ainda não confirmadas) e a do VJT-003/PR #19 (`checklist_templates` deveria chegar a 97 linhas; `trips.num_criancas` nova coluna) |
...
| Branch `claude/vjt-003-onboarding-wizard` (do PR #18 descartado) | **Não foi possível apagar nesta sessão**: `git push origin --delete` retornou 403 (política do proxy de git desta sessão bloqueia deleção de ref, distinto de erro de credencial). PR e issue já estão fechados; a branch órfã precisa ser apagada manualmente no GitHub (Settings → Branches, ou `git push --delete` de um ambiente sem essa restrição) |
| Última atualização desta seção | 2026-07-24 (esta sessão: fechamento da duplicidade VJT-003 + merge do PR #19) |
