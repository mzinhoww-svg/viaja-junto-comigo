...
export function BriefingForm({
  ...
  useEffect(() => {
    if (q.data?.payload) setValues(q.data.payload);
  }, [q.data?.payload]);

  const locked =
    readOnly ||
    q.data?.status === "submitted" ||
    q.data?.status === "in_review" ||
    q.data?.status === "done";

  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.rpc(
        "save_briefing" as never,
        {
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["briefing", requestId, productKey] });
    },
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      const r2 = await supabase.rpc(
        "submit_briefing" as never,
        {
          _request_id: requestId,
          _product_key: productKey,
        } as never,
      ...
      <div className="rounded-2xl bg-white border border-[var(--color-border)] p-5">
        ...
        {locked && q.data?.status !== "done" && (
          <p className="mt-4 text-xs text-ink-soft">
            Briefing enviado — a Letícia já recebeu e vai retornar pelo portal.
          </p>
        )}
        {q.data?.status === "done" && (
...
function StatusBadge({ status }: { status: "draft" | "submitted" | "in_review" | "done" }) {
  ...
  return (
    <span
      className={`text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-full ${map.cls}`}
    >
      {map.label}
    </span>
  );
...
function FieldRender({
  ...
  const base =
    "w-full rounded-xl border border-[var(--color-border)] px-3 py-2 text-sm bg-white disabled:bg-slate-50 disabled:text-ink-soft";
  return (
...
export function BriefingReadOnly({
  requestId,
  productKey,
}: {
  requestId: string;
  ...
  const markReviewed = useMutation({
    mutationFn: async () => {
      if (!q.data?.id) return;
      const { error } = await supabase.rpc(
        "mark_briefing_reviewed" as never,
        { _briefing_id: q.data.id } as never,
      );
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  if (!q.data) {
    return (
      <div className="rounded-2xl border border-dashed border-[var(--color-border)] p-4 text-sm text-ink-soft">
        Cliente ainda não preencheu o briefing.
      </div>
    );
  }
    ...
    <div className="rounded-2xl border border-[var(--color-border)] bg-amber-50/40 p-4">
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
        {meta.fields.map((f) => {
          ...
          return (
            <div key={f.name}>
              <dt className="text-[10px] uppercase tracking-wider text-ink-muted font-bold">
                {f.label}
              </dt>
