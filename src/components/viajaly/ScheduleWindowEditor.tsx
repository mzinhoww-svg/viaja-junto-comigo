...
export function ScheduleWindowEditor() {
  ...
  const saveMut = useMutation({
    mutationFn: async (slots: Slots) => {
      await upsertFn({ data: { slots } });
    },
    onSuccess: () => {
  ...
  function addDate() {
    ...
    if (draftDate < new Date().toISOString().slice(0, 10)) {
    ...
    if (next[draftSvc][draftCons].includes(draftDate)) {
      toast.info("Data já liberada");
      return;
    }
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl border border-[var(--color-border)] p-5">
        ...
        <p className="text-xs text-ink-soft mt-1">
          Os clientes só podem escolher datas dentro das janelas liberadas aqui.
        </p>
              ...
              return (
                <div key={cons} className="rounded-xl border border-[var(--color-border)] p-3">
                  <p className="text-xs font-bold text-ink uppercase tracking-wider">
                    {CONSULATE_LABEL[cons]}
                  </p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {dates.length === 0 && (
                      <span className="text-xs text-ink-muted italic">Sem datas</span>
                    )}
                    {dates.map((d) => (
                      <span
                        key={d}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[var(--color-muted)] text-xs font-semibold text-navy"
                      >
