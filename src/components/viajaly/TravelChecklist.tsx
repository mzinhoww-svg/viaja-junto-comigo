...
export function TravelChecklist({
  ...
  const save = useMutation({
    mutationFn: async (next: Record<string, boolean>) => {
      const { error } = await supabase.rpc("save_travel_checklist", {
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 text-left p-2.5 rounded-xl border transition ${
                  checked
                    ...
                    : "bg-white border-[var(--color-border)] hover:border-coral/40"
                } ${readOnly ? "opacity-70 cursor-default" : ""}`}
              >
                <span
                  className={`w-5 h-5 rounded-full flex items-center justify-center border ${checked ? "bg-emerald-500 border-emerald-500 text-white" : "border-[var(--color-border)]"}`}
                >
                  {checked && <Check size={12} />}
                </span>
                <span
                  className={`text-sm ${checked ? "text-emerald-900 line-through" : "text-ink"}`}
                >
                  {i.label}
                </span>
              </button>
