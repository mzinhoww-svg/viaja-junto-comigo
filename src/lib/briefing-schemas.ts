...
export const DISCLAIMERS: Record<ProductKey, string> = {
  ...
  roteiro:
    "O roteiro é uma sugestão de itinerário; reservas e compras são feitas por você ou com nosso apoio.",
};
...
export type FormField = FieldKind & {
  name: string;
  showIf?: (v: Record<string, unknown>) => boolean;
};

// ── Passaporte ────────────────────────────────────────────────
export const passaporteSchema = z
  .object({
...
export const passaporteFields: FormField[] = [
  ...
  {
    name: "passaporte_anterior_num",
    kind: "text",
    label: "Nº do passaporte anterior",
    showIf: (v) => v.tipo === "renovacao",
    required: true,
  },
  {
    name: "passaporte_anterior_validade",
    kind: "date",
    label: "Validade do anterior",
    showIf: (v) => v.tipo === "renovacao",
  },
  {
    name: "cidade_retirada_pf",
    kind: "text",
    label: "Cidade de retirada (PF)",
    required: true,
  ...
  },
  {
    name: "quinzena_pf",
    kind: "select",
    label: "Quinzena preferida na PF",
    required: true,
    options: [
      { value: "1q", label: "1ª quinzena" },
      { value: "2q", label: "2ª quinzena" },
  ...
  { name: "tem_foto", kind: "boolean", label: "Já tenho foto 5x5 atualizada" },
  {
    ...
    kind: "textarea",
    label: "Observações",
    placeholder: "Restrições de horário, particularidades…",
  },
];
...
export const roteiroSchema = z.object({
  ...
  interesses: z
    .array(z.enum(["gastronomia", "compras", "cultura", "natureza", "parques", "vida_noturna"]))
    .default([]),
...
export const roteiroFields: FormField[] = [
  ...
  {
    name: "criancas_idades",
    kind: "text",
    label: "Idades das crianças",
    showIf: (v) => v.criancas === "sim",
    placeholder: "Ex.: 5, 9",
  },
  {
    name: "estilo",
  ...
  },
  {
    ...
    placeholder: "Alimentares, mobilidade, etc.",
  },
...
export const milhasFields: FormField[] = [
  {
    ...
    options: [
      ...
      { value: "azul", label: "Azul Tudo Azul" },
  ...
  },
  {
    ...
    options: [
      ...
      { value: "resgatar", label: "Resgatar" },
      { value: "transferir", label: "Transferir" },
      { value: "manter_status", label: "Manter status" },
    ],
  },
  {
    name: "trecho_alvo",
    kind: "text",
    label: "Trecho alvo (se houver)",
    placeholder: "Ex.: GRU → MIA, classe executiva",
  },
  { name: "pontos_atuais", kind: "number", label: "Pontos atuais somados (aprox.)", min: 0 },
  {
    name: "saldo_faixa",
    kind: "select",
    label: "Faixa de saldo total",
    required: true,
    options: [
      { value: "<50k", label: "Até 50 mil" },
      { value: "50_200k", label: "50–200 mil" },
      { value: "200k_1M", label: "200 mil – 1 milhão" },
      { value: ">1M", label: "Acima de 1 milhão" },
    ],
  },
  {
    name: "gasto_mensal_faixa",
    kind: "select",
    label: "Gasto mensal em cartão",
    required: true,
    options: [
      { value: "<2k", label: "Até R$ 2k" },
      { value: "2_10k", label: "R$ 2–10k" },
      { value: "10_30k", label: "R$ 10–30k" },
      { value: ">30k", label: "Acima de R$ 30k" },
    ],
  },
  {
    name: "esforco",
    kind: "select",
    label: "Nível de esforço disponível",
    required: true,
    options: [
  ...
  },
  {
    name: "cpfs_familia",
    kind: "textarea",
    label: "CPFs da família (opcional)",
    placeholder: "Para clube/pool familiar",
  },
  {
    name: "cartoes",
    kind: "textarea",
    label: "Cartões que possui",
    placeholder: "Bandeira e categoria",
  },
  {
    name: "aceita_transferencia_bancaria",
    kind: "boolean",
    label: "Aceito bônus por transferência bancária",
  },
];
