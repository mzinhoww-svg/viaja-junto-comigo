...
| Migration aplicada no Supabase | **Sim** — aplicada manualmente via SQL Editor do dashboard em 2026-07-24 (o conector Supabase desta conta não acessa o projeto `urrlqljlibpzaqnemlwf`, gerenciado pelo Lovable Cloud, então a aplicação foi manual). Falta apenas confirmar em produção após o merge do PR #11: `select count(*) from paises_visto` = 14 e `select count(*) from checklist_templates` = 15 |
...
- **Atualização 2026-07-24**: revisão de código do PR #11 concluída (diff completo conferido — migration idêntica à Seção 4, rotas/gate de auth/bottom nav corretos, CI verde, sem achados bloqueantes; comentário de revisão postado no PR já que a auto-aprovação do próprio autor não é permitida pelo GitHub). Migration **aplicada manualmente** via SQL Editor do dashboard Supabase (o conector desta conta não acessa `urrlqljlibpzaqnemlwf`, ver nota na Seção 3). Falta confirmar as contagens de seed em produção e o merge humano do PR #11.
- **Pendências deixadas para tickets futuros**: `VITE_SENTRY_DSN` manual no painel Vercel; confirmação das contagens de seed pós-merge; telas internas (VJT-002+).