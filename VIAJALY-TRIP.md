...
| Campo | Valor |
|---|---|
| Onda atual | 1 — Fundação **parcialmente** concluída: VJT-001, VJT-002 e VJT-008 mergeados (confirmado via estado real dos PRs no GitHub, não suposição). **VJT-003 (wizard de onboarding) NÃO foi mergeado** e está **duplicado**: duas sessões paralelas o executaram de forma independente, gerando issue [#16](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/16)/PR [#18](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/18) (branch `claude/vjt-003-onboarding-wizard`) **e** issue [#17](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/17)/PR [#19](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/19) (branch `claude/vjt-003-wizard-onboarding`) — ver nota abaixo |
...
| Ticket em aberto aguardando review | VJT-003, em **duplicidade**: PR #18 (draft, CI verde, criado 2026-07-24 15:32) e PR #19 (draft, CI verde, criado 2026-07-24 15:34) — ambos fecham "VJT-003" mas em issues diferentes (#16 e #17) e com implementações distintas (rotas `/trip/onboarding` vs `/trip/novo`, migrations diferentes, um deles altera `trips` com `num_criancas`). **Decisão humana pendente**: qual PR mergear (ou como reconciliar os dois) — nenhuma ação de merge/close foi tomada aqui |
| Migration aplicada no Supabase | **Não verificável nesta sessão**: o conector Supabase desta conta só enxerga 3 projetos não relacionados (`crm-ai-studio`, `supabase-purple-book`, `the-loyalty`) — sem acesso a `urrlqljlibpzaqnemlwf` (gerenciado pelo Lovable Cloud). As contagens `select count(*) from paises_visto` = 14 e `select count(*) from checklist_templates` = 15 **não puderam ser confirmadas por consulta direta**; a única evidência indireta é o `types.ts` regenerado pelo Lovable Cloud refletindo as 16 tabelas/funções, o que não prova as contagens de linhas. Verificação manual via SQL Editor do dashboard segue pendente |
...
| Última atualização desta seção | 2026-07-24 (auditoria: verificação direta de PRs via GitHub API + tentativa de consulta ao Supabase/Vercel via conectores) |
> ⚠️ **Duplicidade encontrada nesta sessão (auditoria, não suposição)**: issue #16/PR #18 e issue #17/PR #19 são duas execuções independentes e completas do mesmo ticket VJT-003, abertas por sessões diferentes com ~2 minutos de diferença (issue #16 às 15:21, issue #17 às 15:26; PR #18 às 15:32, PR #19 às 15:34, todos em 2026-07-24). Nenhum dos dois PRs foi mergeado ou fechado por esta sessão — a escolha de qual manter (ou como reconciliar/combinar os dois) é uma decisão humana. Ver Seção 8 para o detalhamento de cada PR.
...
**VJT-007 UI de checklists** — Dá interface aos dados clonados. IN: 4 listas, CRUD, marcos 90/60/30/15/7, progresso por lista e global. Aceite: marcar item atualiza o dashboard sem reload. Bloqueado por: VJT-003, VJT-003b (catálogo premium precisa fechar a faixa 80-120 antes da UI ser considerada "dados completos")
