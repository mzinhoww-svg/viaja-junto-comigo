    ...
    <div className="min-h-screen bg-appbg text-ink">
      ...
      <header className="sticky top-0 z-30 bg-cream/90 backdrop-blur border-b border-[var(--color-border)]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link to="/portal/login" className="text-sm text-ink-soft hover:text-navy hidden sm:inline">Já é cliente? Portal</Link>
      ...
      <section className="bg-navy text-cream anim-vfade">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-14 sm:py-20 text-center">
          <span className="text-xs uppercase tracking-[0.2em] text-coral font-bold">Visto americano</span>
          <h1 className="mt-3 text-3xl sm:text-5xl font-display font-extrabold leading-[1.1] max-w-3xl mx-auto">{hero.h1}</h1>
          <p className="mt-4 text-cream/80 text-base sm:text-lg max-w-2xl mx-auto">{hero.sub}</p>
          <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
            <a href="#form" className="inline-flex items-center justify-center rounded-full bg-coral hover:bg-[var(--color-coral-pressed)] text-cream font-bold px-7 h-12 text-sm">
              {hero.cta}
            </a>
            <a href="#como" className="inline-flex items-center justify-center rounded-full border border-cream/30 hover:border-cream text-cream font-semibold px-7 h-12 text-sm">
              Ver como funciona
            </a>
          </div>
          <p className="mt-5 text-xs text-cream/60">Resposta em até 24h · Consultoria de viagem — não garante a aprovação do visto.</p>
        </div>
      </section>

      {/* Serviços */}
      <section id="servicos" className="max-w-5xl mx-auto px-4 sm:px-6 py-14">
        <h2 className="text-center text-2xl sm:text-3xl font-display font-extrabold text-navy">Soluções de consultoria</h2>
        <div className="mt-8 rounded-3xl border border-[var(--color-border)] bg-white p-6 sm:p-8 grid md:grid-cols-[1.2fr_1fr] gap-6 items-center">
          <div>
            <div className="inline-flex items-center gap-2 text-teal font-bold text-sm"><Stamp size={18} /> Assessoria de Visto EUA</div>
            ...
            <a href="#form" className="mt-6 inline-flex items-center justify-center rounded-full bg-navy hover:bg-[var(--color-navy-light)] text-cream font-semibold px-6 h-11 text-sm">
          ...
          <div className="rounded-2xl bg-cream border border-coral/20 p-6">
            ...
            <div className="mt-3 space-y-2">
              {[
                ...
                <div key={p.label} className="flex items-center gap-3 rounded-xl bg-white border border-[var(--color-border)] p-3">
                  <p.Icon size={18} className="text-coral" />
                  <div><p className="font-display font-bold text-navy text-sm">{p.label}</p><p className="text-[11px] text-ink-soft">{p.sub}</p></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="como" className="bg-white border-y border-[var(--color-border)]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-14">
          <h2 className="text-center text-2xl sm:text-3xl font-display font-extrabold text-navy">Como funciona</h2>
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <div key={n} className="rounded-2xl border border-[var(--color-border)] p-4">
                <span className="w-8 h-8 rounded-full bg-navy text-cream grid place-items-center font-display font-extrabold text-sm">{n}</span>
      ...
      <section className="bg-navy text-cream">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 grid sm:grid-cols-3 gap-8 text-center">
          {[
            { Icon: Users, t: "Especialistas", d: "Quem entende do processo — DS-160 revisado por humano, não por robô." },
            { Icon: ShieldCheck, t: "Segurança", d: "Seus dados tratados com sigilo, em conformidade com a LGPD." },
            { Icon: Smartphone, t: "Tempo real", d: "Tudo num app: proposta, pagamento, documentos e agenda." },
          ].map((p) => (
            <div key={p.t}>
              <p.Icon size={28} className="mx-auto text-coral" />
              <p className="mt-3 font-display font-extrabold uppercase tracking-wider text-sm">{p.t}</p>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-14">
        <p className="text-center text-xs uppercase tracking-[0.2em] text-coral font-bold">O que dizem nossos clientes</p>
        <h2 className="mt-2 text-center text-2xl sm:text-3xl font-display font-extrabold text-navy">Gratidão e muitas viagens</h2>
        <div className="mt-8 grid sm:grid-cols-3 gap-4">
          {[
            { n: "Mariana", c: "São Paulo, SP", q: "Tinha pavor do DS-160. A Viajaly conduziu tudo e a entrevista foi tranquila." },
            { n: "Rafael", c: "Rio de Janeiro, RJ", q: "Acompanhar cada etapa pelo app deu muita segurança. Recomendo demais." },
            { n: "Camila", c: "Belo Horizonte, MG", q: "Atendimento rápido e humano. Deu tudo certo com a família inteira." },
          ].map((t) => (
            <figure key={t.n} className="rounded-2xl border border-[var(--color-border)] bg-white p-5">
              <div className="flex items-center gap-3">
                <span className="w-10 h-10 rounded-full bg-teal/15 text-teal grid place-items-center font-display font-extrabold">{t.n[0]}</span>
                <div><figcaption className="font-display font-bold text-navy text-sm">{t.n}</figcaption><p className="text-[11px] text-ink-muted uppercase tracking-wider">{t.c}</p></div>
              </div>
              <blockquote className="mt-3 text-sm text-ink">“{t.q}”</blockquote>
            </figure>
          ))}
        </div>
        <p className="mt-3 text-center text-[11px] text-ink-muted">Depoimentos ilustrativos — substituir por reais com consentimento.</p>
      </section>

      {/* Formulário (conversão A/B) */}
      <section id="form" className="bg-cream border-t border-[var(--color-border)]">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-14">
          <h2 className="text-center text-2xl sm:text-3xl font-display font-extrabold text-navy">Solicite seu diagnóstico grátis</h2>
          <p className="mt-2 text-center text-ink-soft text-sm">Conta pra gente o que precisa. A Letícia retorna em até 24h.</p>
      ...
      <footer className="bg-navy text-cream/80">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
          <Logo size={26} />
          <p className="mt-3 text-sm text-cream/70 max-w-md">Consultoria de viagem completa — vistos, passaporte, roteiros e milhas. Suporte de ponta a ponta pela Viajaly.</p>
          <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm">
            <a href="#servicos" className="hover:text-cream">Serviços</a>
            <a href="#como" className="hover:text-cream">Como funciona</a>
            <a href={waLink()} target="_blank" rel="noreferrer" className="hover:text-cream">WhatsApp</a>
            <a href="/privacidade" className="hover:text-cream">Política de Privacidade</a>
          </div>
          <p className="mt-6 text-[11px] text-cream/50 leading-relaxed border-t border-cream/10 pt-4">
            A Viajaly presta consultoria de viagem, não jurídica, e não garante a aprovação de vistos — a decisão é exclusiva do consulado dos EUA.
            Não somos órgão do governo e não temos vínculo com consulados ou embaixadas. Taxas governamentais (MRV/visto, passaporte, Polícia Federal) são pagas à parte pelo cliente.
          </p>
        </div>
      </footer>
    </div>
