        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)]">
              ...
              <button onClick={() => setOpen(false)} className="text-ink-soft hover:text-coral">
                <X size={16} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-2">
              ...
              {(q.data ?? []).map((t) => (
                <button
                  key={t.id}
                  onClick={() => pick(t.id)}
                  className="w-full text-left px-3 py-2 rounded-xl hover:bg-[var(--color-muted)] transition"
                >
