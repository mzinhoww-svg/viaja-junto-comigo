...
| Campo | Valor |
|---|---|
| Onda atual | 1 (Fundação) + 2 (Financeiro/Checklists) + 3 (Roteiro+PDF) + 4 (Visto) + 5 (Monetização, parcial) + 6 (Colaboração+IA, parcial) + 7 (Polimento, parcial) fechadas — VJT-001, VJT-002, VJT-003, VJT-003b, VJT-003c, VJT-004, VJT-004b, VJT-005, VJT-006, VJT-007, VJT-007b, VJT-008, VJT-009, VJT-010, VJT-011, VJT-016 e **VJT-014** mergeados (VJT-014 mergeado em 2026-07-25T17:05:31Z, PR #61, confirmado via API do GitHub). **Ainda executável**: VJT-012, VJT-013, VJT-015 e VJT-017 — todos bloqueados só por VJT-011, que já mergeou; nenhum tem issue/PR aberto ainda (a confirmar de novo na próxima sessão antes de despachar, conforme protocolo). VJT-011b (Seção 7, sem bloqueio) também já pode ser pego a qualquer momento |
| Último ticket concluído (mergeado) | VJT-014 (PR [#61](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/61), issue [#60](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/60), mergeado em 2026-07-25T17:05:31Z, confirmado via API do GitHub — `merged: true`, `merged_by: mzinhoww-svg`, commit de merge `99336e2`). Vem depois de VJT-016 (PR #53, mergeado 16:40:54Z) — as duas ondas rodaram em sessões paralelas. Detalhes no Log (Seção 8) |
| Ticket em aberto aguardando review | — nenhum |
| Migration aplicada no Supabase | **Não verificável nesta sessão** (mesma limitação já registrada): o conector Supabase desta conta não acessa `urrlqljlibpzaqnemlwf` (Lovable Cloud). Seguem **cinco migrations pendentes de aplicação manual** via SQL Editor: VJT-001 (contagens `paises_visto`=14/`checklist_templates`=15), VJT-003/PR #19 (`checklist_templates` deveria chegar a 97 linhas; `trips.num_criancas` nova coluna), VJT-003b/PR #24 (`checklist_templates` deveria chegar a 142 linhas), VJT-011 (`ALTER PUBLICATION supabase_realtime ADD TABLE public.entitlements`, agora com guard `DO $$ ... EXCEPTION WHEN duplicate_object` — idempotente, pode ser reaplicada com segurança; sem essa aplicação manual, `useEntitlement()` continua funcionando via leitura normal do React Query, só a invalidação em tempo real fica inativa até a migration ser aplicada, mas o hook tem backstop próprio se o canal cair depois de conectado) e **VJT-014/PR #61** (`increment_ai_usage()` + `ai_daily_message_count()`, ambas `security definer`, mais o índice parcial `idx_ai_messages_created_at_user`; sem essa migration o assistente IA retorna erro na primeira mensagem — é bloqueante para a feature, ao contrário da do VJT-011) — nenhuma contagem confirmada em produção. VJT-005/VJT-006/VJT-007/VJT-009/VJT-010/VJT-016 não têm migration nova |
...
| Última atualização desta seção | 2026-07-25 (VJT-014 mergeado — PR #61 confirmado via API do GitHub, `merged_at` 17:05:31Z; migration do VJT-014 entra como pendente **bloqueante** para o assistente, junto do cadastro dos secrets de Edge Function; VJT-012, VJT-013, VJT-015 e VJT-017 seguem executáveis, sem issue/PR aberto ainda; VJT-011b segue aberto, sem bloqueio) |
...
**VJT-014 Assistente IA** — Diferencial premium e motor de conversão. IN: Edge Function (OpenRouter + DeepSeek — ver Seção 3; a versão original deste ticket dizia Anthropic/Haiku), contexto da trip, cotas 10/100 com contador, escopo fechado (recusa fora do tema, redireciona visto com UTM), kill switch `AI_ENABLED` + teto diário global. Aceite: resposta usa dados reais da trip; msg 11 no free dispara paywall. Bloqueado por: VJT-011
...
### 2026-07-25 — VJT-014 Assistente IA — **Mergeado** (PR [#61](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/61), issue [#60](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/60), mergeado em 2026-07-25T17:05:31Z, confirmado via API do GitHub — `merged: true`, `merged_by: mzinhoww-svg`, commit de merge `99336e2`)
- **Contexto de execução em paralelo**: rodou ao lado do VJT-016 (PR #53/#62, PWA), em sessões separadas. Escopos disjuntos — VJT-014 não tocou manifest/ícones/service worker, VJT-016 não tocou nada de IA. VJT-011 (dependência única) foi confirmado mergeado via API antes de começar, não pelo texto deste arquivo.
- **Edge Function nova `supabase/functions/ai-chat/index.ts`** (Deno): chama a Anthropic no modelo `claude-haiku-4-5` (classe econômica, Seção 3). `ANTHROPIC_API_KEY` lida só de `Deno.env` — **nunca no client nem no Vercel**. Client criado com o **JWT do chamador** (anon key + header `Authorization`), nunca service role: o isolamento entre trips vem da RLS `trips_select`/`is_trip_member` já existente, então `trip_not_found` cobre igualmente "não existe" e "não sou membro".
- **Módulo puro novo `src/lib/ai-assistant.ts`** (+`.test.ts`): `isMessageValid`, `isVisaQuestion`, `buildTripContextBlock`, `buildSystemPrompt` e as constantes de modelo/limites. Sem I/O — mesma convenção de `trip-math.ts`/`entitlements.ts`/`trip-visa.ts`, e importado tanto pelo client quanto pela Edge Function (fonte única, sem duplicar prompt/regra entre os dois lados).
- **Reaproveitamento, não reimplementação**: cotas 10/100 e plano vêm de `entitlements.ts`/`useEntitlement()` do **VJT-011** (`aiMsgLimit`/`isAiQuotaExhausted`) — nenhuma lógica de plano paralela nos dois lados; paywall usa a instância única de `usePaywall()`; contador usa `useAiUsage()`; modo/countdown da trip vêm de `determinarModoTrip`/`calcularMesesRestantes` do **`trip-math.ts`**; link de visto usa `buildVisaConsultoriaLink` do **VJT-009** (`trip-visa.ts` → `whatsapp.ts` → `contact.ts`), **sem criar número ou helper novo**.
- **Escopo fechado**: recusa fora do tema é instruída no prompt de sistema (não determinística — limitação registrada no próprio código). Já o **redirecionamento de visto é determinístico**: `isVisaQuestion` (normaliza acento/caixa) + destino que exige visto ⇒ a função responde direto com o link WhatsApp+UTM e **não gasta chamada à Anthropic**.
- **Kill switch e custo**: secrets `AI_ENABLED` (`"false"` ⇒ 503 em toda chamada) e `AI_DAILY_MSG_CAP` (default 300, teto **global** diário, distinto da cota por usuário — protege contra abuso/bug em loop).
- **Arquivos novos**: `src/lib/ai-assistant.ts`+`.test.ts`, `src/hooks/useAiChat.ts`, `src/components/trip/mais/AiChatSheet.tsx`+`.test.tsx`, `supabase/functions/ai-chat/index.ts`, `supabase/migrations/20260725210000_vjt014_ai_assistant.sql`. Editados: `AiAssistantCard.tsx` (deixa de ser stub, recebe `tripId`)+teste, `MaisDashboard.tsx`, e `whatsapp.ts`/`trip-visa.ts` (só a extensão `.ts` nos imports, para o runtime Deno resolver — **uma linha em cada, zero linha de lógica**, verificado no diff e com os 10 testes do VJT-009 passando sem alteração).
- **Tabelas/RLS**: migration com duas funções `security definer` e **nenhuma tabela ou policy nova**. `increment_ai_usage()` lê `auth.uid()` **internamente** (nunca como parâmetro — um usuário não incrementa a cota alheia) e faz upsert atômico para não perder incrementos concorrentes; `ai_daily_message_count()` devolve **só um número** (precisa ser `security definer` porque `aim_all` restringe leitura a membros da própria trip). Índice parcial `idx_ai_messages_created_at_user` para a contagem diária não virar sequential scan.
- **Verificação local**: bun — lint 0 erros, typecheck 0 erros, test 290/290 (16 novos), build ok. CI do PR verde. **QA manual real (Playwright, 375×812, rede Supabase mockada)** nos 4 critérios de aceite, com asserção explícita de que a cota esgotada **não chega a chamar a Edge Function**.
- **⚠️ Pendência bloqueante para a feature funcionar em produção**: aplicar a migration via SQL Editor **e** cadastrar os secrets (`ANTHROPIC_API_KEY` obrigatório; `AI_ENABLED`/`AI_DAILY_MSG_CAP` opcionais) em Supabase → Edge Functions → Secrets, **e fazer o deploy da função `ai-chat`**. Sem isso o chat retorna erro na primeira mensagem — diferente da migration do VJT-011, que era degradação suave. Mesma limitação de acesso ao projeto Lovable Cloud já registrada na Seção 0.
- **Pendências deixadas para tickets futuros**: evento `ai_message_sent` (VJT-015); histórico de chat renderizado ao reabrir o sheet (a conversa É persistida em `ai_conversations`/`ai_messages` e o contexto do servidor é mantido via `conversation_id`, mas a UI abre em branco).
