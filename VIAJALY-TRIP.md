...
| Campo | Valor |
|---|---|
| Onda atual | 1 (Fundação) + 2 (Financeiro/Checklists, iniciada) — VJT-001, VJT-002, VJT-003, VJT-003b, VJT-003c, VJT-004 e VJT-008 mergeados. **Executáveis agora**: VJT-004b, VJT-005 e VJT-006 (todos bloqueados só por VJT-004, já mergeado). VJT-007 **ainda não** é executável — passou a depender também de VJT-004b (ver achado do log do VJT-004: `MARCOS_CONHECIDOS` duplicado). Recomendação: **VJT-004b primeiro** (pequeno, rápido, evita que VJT-007 redeclare a mesma lista de marcos); VJT-005/VJT-006 (Financeiro) podem rodar em paralelo em sessão separada, sem conflito de arquivos |
| Último ticket concluído (mergeado) | VJT-004 (PR [#25](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/25), issue #22, mergeado em 2026-07-24) — mergeado na mesma sessão que VJT-003b (PR [#24](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/24), issue #21) e VJT-003c (PR [#23](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/23), issue #20). Os três rodaram em paralelo (sessões/ambientes separados, sem overlap de arquivos) e foram revisados diff-a-diff antes do merge — nenhum precisou de ajuste. Detalhes no Log (Seção 8) |
| Ticket em aberto aguardando review | — nenhum |
| Migration aplicada no Supabase | **Não verificável nesta sessão** (mesma limitação já registrada): o conector Supabase desta conta não acessa `urrlqljlibpzaqnemlwf` (Lovable Cloud). Agora há **três migrations pendentes de aplicação manual** via SQL Editor: VJT-001 (contagens `paises_visto`=14/`checklist_templates`=15), VJT-003/PR #19 (`checklist_templates` deveria chegar a 97 linhas; `trips.num_criancas` nova coluna) e VJT-003b/PR #24 (`checklist_templates` deveria chegar a 142 linhas) — nenhuma contagem confirmada em produção |
...
| Última atualização desta seção | 2026-07-24 (esta sessão: revisão diff-a-diff + merge de VJT-003b/PR #24, VJT-003c/PR #23 e VJT-004/PR #25) |
