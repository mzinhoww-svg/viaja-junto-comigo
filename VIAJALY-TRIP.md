...
| Campo | Valor |
|---|---|
| Onda atual | 1 (Fundação) + 2 (Financeiro/Checklists, em andamento) — VJT-001, VJT-002, VJT-003, VJT-003b, VJT-003c, VJT-004, VJT-004b, VJT-005, VJT-008 e VJT-010 mergeados. **Executáveis agora**: VJT-006 e VJT-007 (ambos desbloqueados). Recomendação: **VJT-007** primeiro — sem ambiguidade de design pendente. VJT-006 precisa de uma decisão antes de começar: `/trip/financeiro` (VJT-005/PR #31) foi construída como página de conteúdo único, **sem estrutura de abas** — VJT-006 vai ter que empilhar mais uma seção vertical ou introduzir um componente de abas do zero, decisão de UI não trivial em mobile |
| Último ticket concluído (mergeado) | VJT-005 (PR [#31](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/31), issue #29, mergeado em 2026-07-24) — mergeado na mesma sessão que VJT-004b (PR [#27](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/27), issue #26) e VJT-010 (PR [#30](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/30), issue #28). Os três rodaram em paralelo (sessões separadas, sem overlap de arquivos) e foram revisados diff-a-diff antes do merge — nenhum precisou de ajuste. Detalhes no Log (Seção 8) |
| Ticket em aberto aguardando review | — nenhum |
| Migration aplicada no Supabase | **Não verificável nesta sessão** (mesma limitação já registrada): o conector Supabase desta conta não acessa `urrlqljlibpzaqnemlwf` (Lovable Cloud). Há **três migrations pendentes de aplicação manual** via SQL Editor: VJT-001 (contagens `paises_visto`=14/`checklist_templates`=15), VJT-003/PR #19 (`checklist_templates` deveria chegar a 97 linhas; `trips.num_criancas` nova coluna) e VJT-003b/PR #24 (`checklist_templates` deveria chegar a 142 linhas) — nenhuma contagem confirmada em produção. VJT-005/VJT-010 não têm migration nova |
...
| Última atualização desta seção | 2026-07-24 (esta sessão: revisão diff-a-diff + merge de VJT-004b/PR #27, VJT-010/PR #30 e VJT-005/PR #31) |
