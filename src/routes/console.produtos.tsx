...
function ConsoleProdutos() {
  ...
  return (
    <section className="max-w-3xl anim-vfade">
      <h1 className="text-2xl sm:text-3xl font-display font-extrabold text-navy">
        Produtos &amp; planos
      </h1>
      <p className="text-sm text-ink-soft mt-1">
        Catálogo usado no wizard de orçamento. Alterações refletem em novos orçamentos.
      </p>

      <h2 className="mt-8 mb-2 text-sm font-display font-bold text-navy uppercase tracking-wider">
        Produtos do catálogo
      </h2>
      <div className="space-y-2">
        {products.isLoading && <p className="text-sm text-ink-muted">Carregando…</p>}
        {products.data?.map((p) => (
          <ProductRow key={p.key} product={p} onChange={refreshP} />
        ))}
        <AddProduct onAdded={refreshP} />
      </div>

      <h2 className="mt-10 mb-2 text-sm font-display font-bold text-navy uppercase tracking-wider">
        Planos de visto
      </h2>
      <div className="space-y-2">
        {plans.isLoading && <p className="text-sm text-ink-muted">Carregando…</p>}
        {plans.data?.map((pl) => (
          <PlanRow key={pl.key} plan={pl} onChange={refreshPl} />
        ))}
...
function ProductRow({ product, onChange }: { product: Product; onChange: () => void }) {
  ...
  const dirty =
    name !== product.name || price !== String(product.price) || active !== product.active;

  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("products_catalog")
        .update({ name, price: Number(price.replace(",", ".")) || 0, active })
        .eq("key", product.key as never);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="bg-white rounded-xl border border-[var(--color-border)] p-3 flex flex-wrap items-center gap-2">
      ...
      <Button
        ...
        onClick={() => {
          if (confirm(`Remover "${product.name}"?`)) del.mutate();
        }}
      >
...
function AddProduct({ onAdded }: { onAdded: () => void }) {
  ...
  const add = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("products_catalog").insert({
        key: key.trim(),
        name: name.trim(),
  ...
  return (
    <div className="bg-white rounded-xl border border-dashed border-coral/50 p-3 flex flex-wrap items-center gap-2">
      ...
      <Button
        ...
        disabled={!key.trim() || !name.trim() || add.isPending}
        onClick={() => add.mutate()}
      >
        Criar
      </Button>
...
function PlanRow({ plan, onChange }: { plan: Plan; onChange: () => void }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase
        .from("visto_plans")
        .update({ label, price: Number(price.replace(",", ".")) || 0 })
        .eq("key", plan.key as never);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="bg-white rounded-xl border border-[var(--color-border)] p-3 flex flex-wrap items-center gap-2">
      ...
      <Button
        ...
        disabled={del.isPending}
        onClick={() => {
          if (confirm(`Remover plano "${plan.label}"?`)) del.mutate();
        }}
      >
        <Trash2 size={16} className="text-coral" />
      </Button>
...
function AddPlan({ onAdded }: { onAdded: () => void }) {
  ...
  const add = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("visto_plans").insert({
        key: key.trim(),
        label: label.trim(),
        price: Number(price.replace(",", ".")) || 0,
      } as never);
  ...
  if (!open)
    return (
      <Button size="sm" variant="outline" onClick={() => setOpen(true)}>
        <Plus size={14} className="mr-1" /> Adicionar plano
      </Button>
    );
  return (
    <div className="bg-white rounded-xl border border-dashed border-coral/50 p-3 flex flex-wrap items-center gap-2">
      ...
      <Button
        size="sm"
        className="bg-coral text-cream"
        disabled={!key.trim() || !label.trim() || add.isPending}
        onClick={() => add.mutate()}
      >
        Criar
      </Button>
