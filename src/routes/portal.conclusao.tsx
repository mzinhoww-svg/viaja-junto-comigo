...
function PortalConclusao() {
  ...
  const contacts = useQuery({
    ...
    queryFn: async () => {
      ...
      return (
        (data?.emergency_contacts as { items?: Array<{ label: string; value: string }> } | null)
          ?.items ?? []
      );
    },
  });
  ...
  return (
    <PhoneFrame>
      <div className="px-5 pt-8 pb-24 anim-vfade">
        ...
        <div
          className={`mt-4 p-5 rounded-3xl ${outcome === "aprovado" ? "bg-emerald-600 text-white" : outcome === "recusado" ? "bg-rose-600 text-white" : outcome === "admin_processing" ? "bg-amber-500 text-white" : "bg-navy text-cream"}`}
        >
          ...
          {r.visa_decision_at && (
            <p className="mt-3 text-sm opacity-90">
              Decidido em {new Date(r.visa_decision_at).toLocaleDateString("pt-BR")}
            </p>
          )}
          {outcome === "aprovado" && r.visa_validity_until && (
            <p className="text-sm opacity-90">
              Validade até {new Date(r.visa_validity_until).toLocaleDateString("pt-BR")}
            </p>
          )}
        </div>
        ...
        {!outcome && (
          <p className="mt-4 text-sm text-ink-soft">
            Assim que a Letícia atualizar o resultado da sua entrevista, seu kit de viagem aparece
            aqui.
          </p>
        )}
        ...
        <div className="mt-4">
          <LegalDisclaimer
            variant={outcome === "recusado" || outcome === "admin_processing" ? "strong" : "soft"}
          />
        </div>
        ...
        {outcome === "aprovado" && (
          <div className="mt-5 space-y-4">
            ...
            <FeedbackForm
              requestId={r.id}
              initialRating={r.client_rating ?? null}
              initialFeedback={r.client_feedback ?? null}
            />
          </div>
        )}
