...
export function ScheduleIntentCard({
  ...
  const reopenMut = useMutation({
    mutationFn: async () => {
      await reopenFn({ data: { intent_id: intent.id } });
    },
    onSuccess: () => {
      invalidate();
    ...
    return (
      <div className="rounded-2xl border-2 border-vgreen/40 bg-vgreen/5 p-5">
        <div className="flex items-center justify-between">
          <p className="text-[10px] uppercase tracking-wider text-vgreen font-bold">
            {SERVICE_LABEL[intent.service]}
          </p>
        ...
        {intent.consulate && (
          <p className="mt-1 text-sm text-ink-soft">
            Consulado:{" "}
            <b>
              {CONSULATE_LABEL[intent.consulate as keyof typeof CONSULATE_LABEL] ??
                intent.consulate}
            </b>
          </p>
        )}
    ...
    <div className="bg-white rounded-2xl border border-[var(--color-border)] p-4 md:p-5">
      ...
      {variant === "console" && isConfirmed && (
        <div className="mt-3 text-sm text-navy">
          Confirmado em{" "}
          <b>{new Date(intent.confirmed_date! + "T12:00:00").toLocaleDateString("pt-BR")}</b>
            ...
            <>
              <div className="mt-3 text-sm space-y-1">
                <p className="text-ink-soft">
                  Consulado pedido: <b>{intent.consulate ?? "—"}</b>
                </p>
                <p className="text-ink-soft">
                  Período:{" "}
                  <b>
                    {intent.wish_period
                      ? (PERIOD_LABEL[intent.wish_period] ?? intent.wish_period)
                      : "—"}
                  </b>
                </p>
                <p className="text-ink-soft">
                  Datas:{" "}
                  {intent.wish_dates && intent.wish_dates.length > 0
                    ? intent.wish_dates
              ...
              <div className="mt-4 pt-4 border-t border-[var(--color-border)] flex flex-wrap items-end gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    confirmDate ? confirmMut.mutate(confirmDate) : toast.error("Escolha a data")
                  }
...
function StatusPill({ status }: { status: Intent["status"] }) {
  const map = {
    open: {
      label: "Aguardando preferência",
      cls: "bg-[var(--color-muted)] text-ink-soft",
      Icon: Clock,
    },
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-full ${m.cls}`}
    >
