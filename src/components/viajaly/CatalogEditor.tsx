    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-ink-soft mb-3">
          Nome, preço (R$) e disponibilidade — reflete no wizard de orçamento.
        </p>
        <div className="space-y-2">
          ...
          {products.data?.map((p) => (
            <ProductRow
              key={p.key}
              product={p}
              onSaved={() => qc.invalidateQueries({ queryKey: ["catalog-admin"] })}
            />
          ))}
        </div>
      </div>
      <div>
        ...
        <p className="text-sm text-ink-soft mb-3">
          Start+ / Pro+ / Premium+ — preço por viajante (R$).
        </p>
        <div className="space-y-2">
          ...
          {plans.data?.map((pl) => (
            <PlanRow
              key={pl.key}
              plan={pl}
              onSaved={() => qc.invalidateQueries({ queryKey: ["plans-admin"] })}
            />
          ))}
        </div>
      </div>
