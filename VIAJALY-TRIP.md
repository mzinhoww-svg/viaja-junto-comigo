...
| Campo | Valor |
|---|---|
| Onda atual | 1 (Fundação) + 2 (Financeiro/Checklists) + 3 (Roteiro+PDF) + 4 (Visto) + 5 (Monetização, parcial) + 7 (Polimento, parcial) fechadas — VJT-001, VJT-002, VJT-003, VJT-003b, VJT-003c, VJT-004, VJT-004b, VJT-005, VJT-006, VJT-007, VJT-007b, VJT-008, VJT-009, VJT-010, VJT-011 e **VJT-016** mergeados (VJT-016 mergeado em 2026-07-25T16:40:54Z, PR #53, confirmado via API do GitHub). **Divergência encontrada ao reconciliar esta seção durante o merge de `main` nesta sessão**: `git log origin/main` mostra PR #61 ("VJT-014 assistente IA com contexto da trip e cotas", commit de merge `99336e2`) já mergeado, mas **nenhuma entrada no Log (Seção 8) documenta esse ticket** — nem esta sessão nem a de VJT-014 registraram os detalhes; fica como pendência para a próxima sessão reconciliar de verdade (ler o diff mergeado de PR #61 e escrever a entrada retroativa), já que esta sessão não implementou VJT-014 e não tem como descrever o que foi feito sem inventar. **VJT-012** (este ticket) tem PR [#55](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/55) aberto aguardando review (ver linha abaixo e Log). **Executável agora** (sem issue/PR ainda, a reconfirmar antes de despachar): VJT-013, VJT-015, VJT-017 — todos bloqueados só por VJT-011, que já mergeou. VJT-011b (Seção 7, sem bloqueio) também já pode ser pego a qualquer momento |
| Último ticket concluído (mergeado) | VJT-016 (PR [#53](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/53), issue [#52](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/52), mergeado em 2026-07-25T16:40:54Z, confirmado via API do GitHub — `merged: true`, `merged_by: mzinhoww-svg`, commit de merge `670b918`). Detalhes no Log (Seção 8). VJT-014 (PR #61) também já está em `main` via `git log`, mas sem entrada de log — ver nota de divergência na linha acima |
| Ticket em aberto aguardando review | **VJT-012** Checkout Stripe + webhook (issue [#54](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/54), PR [#55](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/55), branch `claude/stripe-checkout-webhook-vjt-012-5ne0m6`) — ver Log (Seção 8) |
| Migration aplicada no Supabase | **Não verificável nesta sessão** (mesma limitação já registrada): o conector Supabase desta conta não acessa `urrlqljlibpzaqnemlwf` (Lovable Cloud). Seguem **quatro migrations pendentes de aplicação manual** via SQL Editor: VJT-001 (contagens `paises_visto`=14/`checklist_templates`=15), VJT-003/PR #19 (`checklist_templates` deveria chegar a 97 linhas; `trips.num_criancas` nova coluna), VJT-003b/PR #24 (`checklist_templates` deveria chegar a 142 linhas) e VJT-011 (`ALTER PUBLICATION supabase_realtime ADD TABLE public.entitlements`, agora com guard `DO $$ ... EXCEPTION WHEN duplicate_object` — idempotente, pode ser reaplicada com segurança; sem essa aplicação manual, `useEntitlement()` continua funcionando via leitura normal do React Query, só a invalidação em tempo real fica inativa até a migration ser aplicada, mas o hook tem backstop próprio se o canal cair depois de conectado) — nenhuma contagem confirmada em produção. VJT-005/VJT-006/VJT-007/VJT-009/VJT-010 não têm migration nova |
...
| Última atualização desta seção | 2026-07-25 (reconciliado ao dar merge de `main` na branch do VJT-012: VJT-016 mergeado — PR #53 — e VJT-014 já em `main` via PR #61, mas sem entrada de log, ver divergência acima; VJT-012 segue com PR #55 aberto aguardando review; VJT-013/015/017 seguem executáveis sem issue/PR ainda; VJT-011b segue aberto, sem bloqueio) |
...
### 2026-07-25 — VJT-012 Checkout Stripe + webhook de ativação do Premium — **PR aberto, aguardando review** (issue [#54](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/54), PR [#55](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/55))
