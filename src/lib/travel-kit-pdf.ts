...
export function generateTravelKitPDF(d: KitData) {
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Viajante:", 50, y);
  doc.setFont("helvetica", "normal");
  doc.text(d.clientName, 120, y);
  y += 18;
  doc.setFont("helvetica", "bold");
  doc.text("Código:", 50, y);
  doc.setFont("helvetica", "normal");
  doc.text(d.accessCode, 120, y);
  y += 24;
  ...
  doc.text("Resultado do visto", 50, y);
  y += 18;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.setTextColor(40, 40, 40);
  doc.text(`Status: ${d.outcome ?? "Aguardando"}`, 50, y);
  y += 14;
  if (d.decisionAt) {
    doc.text(`Decisão: ${new Date(d.decisionAt).toLocaleDateString("pt-BR")}`, 50, y);
    y += 14;
  }
  if (d.validityUntil) {
    doc.text(`Validade até: ${new Date(d.validityUntil).toLocaleDateString("pt-BR")}`, 50, y);
    y += 14;
  }
  y += 12;
  ...
  doc.setFontSize(13);
  doc.setTextColor(11, 35, 79);
  doc.text("Checklist de viagem", 50, y);
  y += 18;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.setTextColor(40, 40, 40);
  ...
  if (d.emergencyContacts.length > 0) {
    ...
    doc.setFontSize(13);
    doc.setTextColor(11, 35, 79);
    doc.text("Contatos de emergência", 50, y);
    y += 18;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    doc.setTextColor(40, 40, 40);
    d.emergencyContacts.forEach((c) => {
      doc.setFont("helvetica", "bold");
      doc.text(`${c.label}:`, 50, y);
      doc.setFont("helvetica", "normal");
      doc.text(c.value, 160, y);
      y += 16;
      if (y > 760) {
        doc.addPage();
        y = 50;
  ...
  doc.setTextColor(120, 120, 120);
  const disclaimer =
    "A Viajaly presta consultoria de viagem, não jurídica e não garante a aprovação de vistos. A decisão final é sempre do consulado.";
