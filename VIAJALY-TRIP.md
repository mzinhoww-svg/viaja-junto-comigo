...
| Campo | Valor |
|---|---|
| Onda atual | 1 (Fundação) + 2 (Financeiro/Checklists) + 3 (Roteiro+PDF) + 4 (Visto) fechadas — VJT-001, VJT-002, VJT-003, VJT-003b, VJT-003c, VJT-004, VJT-004b, VJT-005, VJT-006, VJT-007, VJT-008, VJT-009 e VJT-010 mergeados. **Executável agora**: **VJT-011** (Onda 5, Monetização) — suas 4 dependências (VJT-005, VJT-006, VJT-009, VJT-010) estão todas mergeadas, confirmado via API do GitHub (não só pelo texto deste arquivo); sem issue/PR existente para VJT-011 (busca feita nesta sessão, `search_issues`/`search_pull_requests` por "VJT-011", zero resultados). VJT-007b (PR #47) segue aberto (draft) mas **não bloqueia** VJT-011 nem nenhum outro ticket da Seção 7 — é um chore isolado de consolidação de chave de invalidação, sem dependência reversa no backlog |
| Último ticket concluído (mergeado) | VJT-009 (PR [#49](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/49), issue #48, mergeado em 2026-07-25). Consolidado nesta sessão junto com dois tickets que já estavam mergeados mas ainda sem entrada no Log: VJT-006 (PR [#33](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/33), issue #32, mergeado em 2026-07-24) e a confirmação retroativa de VJT-007 (PR [#39](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/39), já registrado). Todos os merges e datas foram conferidos direto na API do GitHub nesta sessão, não assumidos. Detalhes no Log (Seção 8) |
| Ticket em aberto aguardando review | VJT-007b (PR [#47](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/47), issue #45) — draft, aberto desde 2026-07-25. Chore de consolidação de chave de invalidação em `useTripChecklists.ts` (reaproveitar `tripDashboardQueryKey` de `trip-query-keys.ts`); escopo isolado do VJT-009 (rodaram em paralelo sem overlap de arquivo) |
| Migration aplicada no Supabase | **Não verificável nesta sessão** (mesma limitação já registrada): o conector Supabase desta conta não acessa `urrlqljlibpzaqnemlwf` (Lovable Cloud). Seguem **três migrations pendentes de aplicação manual** via SQL Editor: VJT-001 (contagens `paises_visto`=14/`checklist_templates`=15), VJT-003/PR #19 (`checklist_templates` deveria chegar a 97 linhas; `trips.num_criancas` nova coluna) e VJT-003b/PR #24 (`checklist_templates` deveria chegar a 142 linhas) — nenhuma contagem confirmada em produção. VJT-005/VJT-006/VJT-007/VJT-009/VJT-010 não têm migration nova |
...
| Última atualização desta seção | 2026-07-25 (esta sessão: pausa do revisor automatizado após 3 hipóteses sem sucesso) |
| Última atualização desta seção | 2026-07-25 (esta sessão: consolidação de onda — Log com entradas novas de VJT-006 e VJT-009; VJT-011 determinado como próximo executável, ainda sem issue/PR aberto) |
