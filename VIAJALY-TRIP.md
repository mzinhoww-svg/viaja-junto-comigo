...
- **Testes**: `resolveEntitlement` (sem linha/premium ativo/premium expirado) + os 6 gatilhos cobertos individualmente (free bate, premium não bate) + realtime (mudança na linha invalida sem reload) + modal único reaproveitado entre gatilhos diferentes. **QA manual real via Playwright** (viewport 375×812, rede Supabase mockada com `Content-Range`/CORS corretos) nas rotas `/trip/mais`, `/trip/roteiro`, `/trip/checklists`, `/trip/novo` — os 6 paywalls confirmados abrindo com o copy certo na aplicação rodando de verdade (`vite dev`), não só em testes unitários; sem erros de console relevantes.
- **Verificação local**: bun 1.3.11 — lint 0 erros (revertidas 4 reformatações incidentais fora de escopo em `src/routes/[.mcp]/*`/`[.well-known]/*`, mesmo padrão do VJT-004b), typecheck 0 erros, test 270/270 (52 novos), build ok.
- **Como testar no celular**: ver corpo do PR #51 (seção "Como testar no celular").
- **Pendências deixadas para tickets futuros**: checkout Stripe real (VJT-012, CTA do modal fica desabilitado "em breve"); convite real de membro e assistente IA real (VJT-013/VJT-014, ambos com aviso "em breve" quando o gate já libera); coluna de linhagem tier em `checklist_items` (teaser premium usa contagem do catálogo completo, não da trip específica — decisão aceita, documentada no código); aplicação manual da migration deste ticket em produção (sem ela, a invalidação em tempo real "sem reload" fica inativa, mas o app continua funcional via refetch normal do React Query).
