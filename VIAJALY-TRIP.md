...
| Última atualização desta seção | 2026-07-25 (consolidação de onda: VJT-012 (PR #55), VJT-013 (PR #59), VJT-014 (PR #61) e VJT-017 (PR #57) mergeados quase simultaneamente por sessões paralelas — todos confirmados via API do GitHub, entradas de log escritas a partir do corpo real de cada PR; único ticket ainda executável da Seção 7 é VJT-015; VJT-011b segue aberto, sem bloqueio) |
...
**VJT-017b Nomear subprocessadores no consentimento LGPD + tornar o gate sensível à versão** — O VJT-017 entregou o consentimento, mas com dois gaps que a troca do provider de IA para OpenRouter + DeepSeek (PR #63) tornou concretos: (1) nem `LGPD_CONSENT_TEXT` nem a seção Compartilhamento de `/privacidade` nomeiam qualquer subprocessador, e a própria categoria de tratamento "inferência de IA sobre dados da viagem" não existe em nenhum dos dois textos ("hospedagem, mensageria" não a descreve); (2) `useLgpdConsent` decide por existência de linha — `versao_termos` é gravado e nunca lido, e `user_lgpd_consents` tem `user_id` como PK, então subir a versão do texto não re-apresenta o aceite para quem já consentiu, exatamente a população que precisa ser informada do subprocessador novo. IN: nomear OpenRouter (EUA) e DeepSeek (China) — além de Supabase e Stripe — no consentimento e na Política, com a menção de transferência internacional; migration trocando a PK por log append-only (`id` próprio + unique `(user_id, versao_termos)`, sem policy de update/delete); gate comparando `versao_termos` com `LGPD_CONSENT_VERSION`; bump para `v2` forçando re-consentimento da base existente (decisão do dono do produto: a base é ~zero hoje, o custo é nulo agora e alto depois). OUT: Edge Function `ai-chat`/provider (PR #63); reescrita da Política além das seções de compartilhamento/IA/transferência. Aceite: consentimento e `/privacidade` nomeiam os subprocessadores e descrevem o tratamento por IA; usuário que aceitou `v1` é re-apresentado ao gate e o aceite antigo continua no log (testado). **Bloqueia o deploy da Edge Function `ai-chat`** — depois do deploy, dado de viagem trafega para OpenRouter/DeepSeek sob consentimento que não cobre esse tratamento. Bloqueado por: —
