...
export function MilhasCardPortal({ requestId }: { requestId: string }) {
    ...
    return (
      <div className="rounded-2xl bg-white border border-[var(--color-border)] p-5">
        <h3 className="font-display font-bold text-navy flex items-center gap-2">
          <Sparkles size={18} className="text-coral" /> Milhas
        </h3>
        <p className="text-sm text-ink-soft mt-2">
          Estamos preparando seu plano de milhas. Você verá aqui assim que for publicado.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-white border border-[var(--color-border)] p-5">
      <h3 className="font-display font-bold text-navy flex items-center gap-2 text-lg">
        <Sparkles size={18} className="text-coral" /> Plano de milhas
      </h3>
...
export function MilhasCardConsole({ requestId }: { requestId: string }) {
  ...
  useEffect(() => {
    if (mil) {
      setPlano(mil.plano ?? "");
      setAlertas(mil.alertas ?? []);
      setAnexos(mil.anexos ?? []);
    }
  }, [mil]);
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["milhas", requestId] });
    },
  ...
  async function handleFile(file: File) {
    if (file.size > 16 * 1024 * 1024) {
      toast.error("Arquivo maior que 16 MB");
      return;
    }
    ...
    <div className="rounded-2xl bg-white border border-[var(--color-border)] p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-ink-soft">
          {mil?.status === "ativo" ? "Ativo (publicado)" : "Em briefing"}
        </span>
      </div>
      ...
      <div>
        ...
        <div className="space-y-1 mb-2">
          {alertas.map((a, i) => (
            <div
              key={i}
              className="flex items-center justify-between gap-2 text-sm bg-amber-50 border border-amber-200 px-3 py-2 rounded-lg"
            >
              <span className="text-amber-900">{a}</span>
              <button
                onClick={() => setAlertas((arr) => arr.filter((_, j) => j !== i))}
                className="text-amber-700 hover:text-coral"
              >
                <X size={14} />
              </button>
            </div>
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            onClick={() => {
              if (novoAlerta.trim()) {
                setAlertas((a) => [...a, novoAlerta.trim()]);
                setNovoAlerta("");
              }
            }}
          >
      ...
      <div>
        ...
        <div className="space-y-1">
          {anexos.map((a, i) => (
            <div
              key={i}
              className="flex items-center justify-between gap-2 text-sm bg-[var(--color-muted)] px-3 py-2 rounded-lg"
            >
              <button
                onClick={() => viewSigned(a.path)}
                className="text-ink hover:text-coral truncate"
              >
                {a.name}
              </button>
              <button
                onClick={() => setAnexos((arr) => arr.filter((_, j) => j !== i))}
                className="text-ink-muted hover:text-coral text-xs"
              >
                remover
              </button>
            </div>
