...
function TemplatesPage() {
  ...
  const save = useMutation({
    mutationFn: async (t: Tpl) => {
      ...
      if (t.id) {
        const { error } = await supabase
          .from("message_templates")
          .update({ title: t.title, category: t.category, body: t.body })
          .eq("id", t.id);
        if (error) throw error;
      } else {
    ...
    onSuccess: () => {
      setEditing(null);
      qc.invalidateQueries({ queryKey: ["msg-templates-admin"] });
  ...
  return (
    <section>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        ...
        <Button
          onClick={() => setEditing({ id: "", title: "", category: "geral", body: "" })}
      ...
      {editing && (
        ...
        >
          <div
            className="bg-white rounded-2xl w-full max-w-lg p-5"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-display font-bold text-navy mb-4">
              {editing.id ? "Editar" : "Novo"} template
            </h3>
