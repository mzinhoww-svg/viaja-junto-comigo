  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, service, status, confirmed_date, consulate, traveler:travelers(name, request_id, request:requests(lead_name))",
        )
        .eq("status", "confirmed")
      ...
      <div className="grid grid-cols-7 gap-1 mt-1">
        ...
        {Array.from({ length: total }).map((_, i) => {
          ...
          return (
            <div
              key={day}
              className={`min-h-[72px] rounded-lg border p-1.5 ${items.length ? "border-coral/40 bg-coral/5" : "border-[var(--color-border)] bg-white"}`}
            >
              ...
              <div className="mt-1 space-y-1">
                {items.slice(0, 3).map((r) => (
                  ...
                  >
                    {SERVICE_SHORT[r.service]} ·{" "}
                    {r.traveler?.request?.lead_name?.split(" ")[0] ?? ""}
                  </Link>
                ))}
                {items.length > 3 && (
                  <p className="text-[10px] text-ink-muted">+{items.length - 3}</p>
                )}
              </div>
            </div>
          );
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("schedule_intents")
        .select(
          "id, service, status, wish_dates, consulate, traveler:travelers(name, request_id, request:requests(lead_name))",
        )
        .eq("status", "sent")
          ...
          <li key={r.id}>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded ${SERVICE_COLOR[r.service]}`}
                >
                  {SERVICE_SHORT[r.service]}
                </span>
              </div>
              <p className="text-xs text-ink-soft mt-0.5">
                ...
                {r.wish_dates && r.wish_dates.length > 0 && (
                  <>
                    {" "}
                    · pediu{" "}
                    {r.wish_dates
