  ...
  head: () => ({
    meta: [
      { title: "Pipeline — Viajaly Console" },
      {
        name: "description",
        content:
          "Pipeline de atendimento Viajaly: organize orçamentos, propostas e agendamentos da equipe em um só painel administrativo.",
      },
      { name: "robots", content: "noindex,nofollow" },
      { property: "og:title", content: "Pipeline — Viajaly Console" },
      {
        property: "og:description",
        content: "Painel administrativo Viajaly para gestão da operação de vistos.",
      },
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("requests")
        .select(
          "id, lead_name, lead_email, access_code, proposal_status, payment_status, tax_status, contract_signed, created_at, visa_outcome, archived_at, assigned_to, lead_source",
        )
    ...
    <section>
      ...
      <div className="flex flex-wrap gap-2 items-center mb-4">
        ...
        <label className="inline-flex items-center gap-2 text-sm text-ink-soft cursor-pointer">
          <input type="checkbox" checked={mine} onChange={(e) => setMine(e.target.checked)} /> Meus
          casos
        </label>
            ...
            <table className="w-full text-sm min-w-[720px]">
              ...
              <tbody>
                ...
                {rows.map((r) => (
                  <tr
                    key={r.id}
                    className={`border-t border-[var(--color-border)] hover:bg-[var(--color-muted)]/50 ${r.archived_at ? "opacity-60" : ""}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <StatusPill variant={r.proposal_status === "accepted" ? "done" : "info"}>
                        {r.proposal_status}
                      </StatusPill>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link
                        ...
                        params={{ id: r.id }}
                        className="text-coral font-semibold hover:underline whitespace-nowrap"
                      >
                        Abrir →
                      </Link>
                    </td>
      ...
      <div className="flex gap-3 min-w-[900px]">
        {STAGES.map((st) => {
          ...
          return (
            <div
              key={st.key}
              className="flex-1 min-w-[180px] bg-[var(--color-muted)]/40 rounded-2xl p-2"
            >
              <div className="px-2 py-1.5 flex items-center justify-between">
                <span className="text-xs font-display font-bold text-navy uppercase tracking-wider">
                  {st.label}
                </span>
