...
| Campo | Valor |
|---|---|
| Onda atual | 1 (Fundação) + 2 (Financeiro/Checklists) + 3 (Roteiro+PDF) + 4 (Visto) + 5 (Monetização) + 6 (Diferenciais) + 7 (Polimento) fechadas — VJT-001 a VJT-011, VJT-012, VJT-013, VJT-014, VJT-016 e VJT-017 mergeados (confirmado via API do GitHub nesta sessão, ao reconciliar a Seção 0/Log depois de notar `main` avançar com múltiplos merges paralelos enquanto o PR #55 do VJT-012 estava aberto: PR #61/VJT-014 mergeado 17:05:31Z, PR #59/VJT-013 mergeado 17:21:55Z, PR #57/VJT-017 mergeado 17:22:06Z, PR #55/VJT-012 mergeado 17:22:20Z — os quatro nos últimos ~17 min da mesma sessão de trabalho, cada um por uma sessão/ambiente diferente). VJT-015 (Instrumentação PostHog) foi executado nesta sessão e está **em review** (issue [#66](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/66), PR [#68](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/68)) — com ele mergeado, a Seção 7 fica sem nenhum ticket de feature pendente e VJT-018 (QA mobile) destrava. VJT-011b (sem bloqueio) segue disponível, assim como VJT-003c e VJT-007b |
| Último ticket concluído (mergeado) | VJT-012 (PR [#55](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/55), issue [#54](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/54), mergeado em 2026-07-25T17:22:20Z, confirmado via API do GitHub — `merged: true`, `merged_by: mzinhoww-svg`, commit de merge `dc27c5f`) — o mais recente por `merged_at` entre os 4 merges quase simultâneos acima, não por ordem de execução. Detalhes de cada um no Log (Seção 8): VJT-012 (entrada já existente, só o cabeçalho mudou de "PR aberto" para "Mergeado"), e entradas novas para VJT-013/VJT-014/VJT-017 escritas nesta sessão a partir do corpo real de cada PR (via API do GitHub), não inventadas — esta sessão não implementou esses três tickets |
| Ticket em aberto aguardando review | **VJT-015 Instrumentação PostHog** — issue [#66](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/66), PR [#68](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/68) (draft, `claude/posthog-vjt-015-mj9gmj`). Sem migration. Detalhes na entrada do Log abaixo |
| Migration aplicada no Supabase | **Não verificável nesta sessão** (mesma limitação já registrada): o conector Supabase desta conta não acessa `urrlqljlibpzaqnemlwf` (Lovable Cloud). Pendentes de aplicação manual via SQL Editor, na ordem: VJT-001 (contagens `paises_visto`=14/`checklist_templates`=15), VJT-003/PR #19 (`checklist_templates` 97 linhas; `trips.num_criancas`), VJT-003b/PR #24 (`checklist_templates` 142 linhas), VJT-011 (`ALTER PUBLICATION supabase_realtime ADD TABLE public.entitlements`, idempotente), e as **quatro novas desta onda** (VJT-012/013/014/017, nenhuma aplicada ainda): `20260725200000_vjt012_premium_checkout_idempotency.sql` (índice único em `entitlements.stripe_payment_id`); `20260725200000_vjt013_trips_select_owner_bootstrap.sql` (**corrige bug de RLS pré-existente desde o VJT-001** — `trips_select` bloqueava o próprio dono de ver a trip que acabou de criar, antes do bootstrap de `trip_members`; sem aplicar, criar trip via `useCreateTrip.ts` pode falhar); `20260725210000_vjt014_ai_assistant.sql` (2 funções `security definer`, sem tabela/policy nova); `20260725200000_vjt017_lgpd_nps_account_deletion.sql` (FK de `savings_entries.created_by` vira `SET NULL`, `ai_conversations`/`trip_invites` viram `CASCADE`, cria `trip_nps_responses`/`user_lgpd_consents`). Nenhuma contagem/aplicação confirmada em produção. VJT-005/VJT-006/VJT-007/VJT-009/VJT-010/VJT-016 não têm migration nova |
| ⚠️ Consentimento LGPD vs. subprocessadores de IA (**VJT-017b**, issue [#65](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/65)) | **Bloqueante para o deploy da `ai-chat`, não só para o merge.** Com a troca para OpenRouter + DeepSeek (PR #63), o contexto da trip passa a transitar por dois terceiros novos, mas nem `LGPD_CONSENT_TEXT` nem a seção Compartilhamento de `/privacidade` nomeiam subprocessadores — e as categorias que a Política lista ("hospedagem, mensageria") não descrevem inferência de IA, então esse tratamento não está coberto em lugar nenhum. Pior: `useLgpdConsent` decide só por existência de linha e **nunca lê `versao_termos`**, então subir a versão do termo não re-apresenta o aceite a quem já consentiu — só a usuários novos. Editar apenas o texto daria falsa sensação de conformidade. **Depois do deploy da `ai-chat`, dado de viagem começa a trafegar sob um consentimento que não cobre esse tratamento** — resolver VJT-017b antes de subir a função, ou aceitar conscientemente o risco e registrar a decisão aqui |
...
| `VITE_POSTHOG_KEY` no Vercel (VJT-015) | **Não cadastrada** — mesma limitação do `VITE_SENTRY_DSN` acima (o conector Vercel não escreve variáveis). Enquanto a chave não existir, o app roda normalmente e **nenhum** evento é enviado (opt-in, igual ao Sentry). Cadastrar no painel: `VITE_POSTHOG_KEY` (obrigatória), `VITE_POSTHOG_HOST` (opcional, default `https://us.i.posthog.com`), `VITE_ANALYTICS_ENABLED` (opcional, `"false"` é o kill switch). Os 2 funis (grátis→compra, visto→lead) precisam ser criados manualmente no painel do PostHog a partir dos nomes de evento de `src/lib/trip-analytics.ts` |
...
| Última atualização desta seção | 2026-07-25 (consolidação de onda: VJT-012 (PR #55), VJT-013 (PR #59), VJT-014 (PR #61) e VJT-017 (PR #57) mergeados quase simultaneamente por sessões paralelas — todos confirmados via API do GitHub, entradas de log escritas a partir do corpo real de cada PR; VJT-011b segue aberto, sem bloqueio). **Adendo pós-consolidação**: PR #63 troca o provider de IA para OpenRouter + DeepSeek (decisão de custo do dono do produto) e, com isso, abre **VJT-017b** (issue #65) — o consentimento LGPD não nomeia os subprocessadores novos e o gate de consentimento ignora a versão do termo; ver a linha destacada acima, que é bloqueante para o deploy da `ai-chat`. **VJT-015 executado e enviado para review** (PR #68 draft): antes de começar, confirmado via API que os únicos PRs abertos eram #63 e #64 (fixture de teste de RLS, follow-up do VJT-013) — nenhum ticket de feature em paralelo, então este ticket, que toca eventos em quase todo o app, rodou sozinho. Durante a execução o VJT-017b abriu o PR #67, criando um conflito de premissa com a âncora do evento `signup` — resolvido dentro do próprio VJT-015 (ver a entrada do Log) |
