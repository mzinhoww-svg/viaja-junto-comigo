...
| Migrations aplicadas no Supabase | **Verificado em produção em 2026-07-25** — pela primeira vez desde o VJT-001 esta linha registra estado conferido, não suposição. As 7 migrations acumuladas foram aplicadas manualmente via SQL Editor do Lovable Cloud (`urrlqljlibpzaqnemlwf`) num **único script transacional**, com pré-verificação e pós-verificação por query de catálogo (`pg_policies`/`pg_proc`/`pg_constraint`/`pg_publication_tables`/`information_schema`) no formato `item \| esperado \| encontrado`: **27/27 linhas com `encontrado = esperado`**. **Aplicadas**: VJT-001 (`paises_visto`=14); VJT-003 (`trips.num_criancas`); VJT-003b (`checklist_templates`=**142** — assinatura final; era 15 antes, confirmando que nenhuma reaplicação duplicou catálogo); VJT-011 (`entitlements` na publication `supabase_realtime`); VJT-012 (índice único parcial `idx_entitlements_stripe_payment_id`, com 0 `stripe_payment_id` duplicados conferidos antes); VJT-013 (**as duas metades juntas**: `trips_select` com `or owner_id = auth.uid()` e `trips_update` com `with check` via `trip_owner_id()`); VJT-014 (`increment_ai_usage()` + `ai_daily_message_count()` + `idx_ai_messages_created_at_user`); VJT-017 (`savings_entries.created_by` nullable com `on delete set null`, `ai_conversations_created_by_fkey`/`trip_invites_created_by_fkey`/`trip_invites_accepted_by_fkey` com `cascade`, tabelas `trip_nps_responses` e `user_lgpd_consents` criadas com RLS). **Única migration ainda pendente de aplicação: `20260725220000_vjt017b_lgpd_consent_history.sql`** — o PR [#67](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/67) já mergeou (2026-07-25T19:57:33Z, commit `2714fa7`), então ela **já está na `main` e não depende mais de merge nenhum**; falta apenas rodá-la no SQL Editor. Ela converte `user_lgpd_consents` em log append-only (derruba a PK de `user_id`, cria `id uuid`, `unique (user_id, versao_termos)`, índice `(user_id, aceito_em desc)`, remove o default `'v1'`) e exige o VJT-017 aplicado antes — **pré-requisito agora satisfeito**. VJT-005/VJT-006/VJT-007/VJT-009/VJT-010/VJT-016 não têm migration |
| ⚠️ Consentimento LGPD vs. subprocessadores de IA (**VJT-017b**, issue [#65](https://github.com/mzinhoww-svg/viaja-junto-comigo/issues/65), PR [#67](https://github.com/mzinhoww-svg/viaja-junto-comigo/pull/67)) | **PR #67 mergeado em 2026-07-25T19:57:33Z** (confirmado via API do GitHub — `merged: true`, `merged_by: mzinhoww-svg`, commit de merge `2714fa7`). O código já está na `main`: `LGPD_CONSENT_TEXT` e `/privacidade` passam a nomear Supabase/Stripe/OpenRouter/DeepSeek com país e a categoria "inferência de IA" (incluindo transferência internacional, Cap. V da LGPD), `LGPD_CONSENT_VERSION` sobe para `v2`, e `useLgpdConsent` passa a comparar `versao_termos` em vez de decidir só por existência de linha — que era o bug central. A ressalva de procedência registrada no corpo do #67 (texto nomeia OpenRouter/DeepSeek enquanto a `main` ainda chamaria a Anthropic) **está resolvida**: o PR #63 mergeou antes, então texto e código apontam para o mesmo provider. **Ainda bloqueante para o deploy da `ai-chat`, agora por um motivo só**: a migration `20260725220000_vjt017b_lgpd_consent_history.sql` não foi aplicada — sem ela `user_lgpd_consents` continua com `user_id` como PK e o gate versionado não tem onde gravar o histórico de aceites. Sequência que falta: **aplicar a migration → deployar a `ai-chat`** |
...
| Última atualização desta seção | 2026-07-25 (**aplicação das migrations pendentes no Supabase**). As 7 migrations acumuladas desde o VJT-003 foram aplicadas em produção via SQL Editor do Lovable Cloud, num único script transacional montado a partir dos arquivos reais de `supabase/migrations/` (conteúdo verbatim, conferido byte a byte com `diff` contra cada arquivo de origem), com guarda de pré-condição e bloco de verificação no fim. Resultado conferido: 27/27 itens com `encontrado = esperado`, `checklist_templates`=142, 8 vereditos `APLICADA`. A linha "Migration aplicada no Supabase" deixa de dizer "não verificável nesta sessão" — o conector Supabase desta conta continua sem alcançar `urrlqljlibpzaqnemlwf`, mas o estado foi verificado por SQL executado no painel e registrado aqui. Também reconciliado nesta edição: **PR #67 (VJT-017b) mergeou às 19:57:33Z**, o que deixa `20260725220000_vjt017b_lgpd_consent_history.sql` como a única migration pendente de aplicação — pendente de execução, não de merge |
